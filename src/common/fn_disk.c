...
uint8_t fn_disk_mount(uint8_t slot, const char *uri, uint8_t readonly,
                      ...
                      fn_disk_info_t *info)
{
    uint8_t request[FN_MAX_PACKET_SIZE];
    uint8_t reply[FN_MAX_PACKET_SIZE];
    uint16_t uri_length;
    ...
    request[0] = FN_DISK_PROTOCOL_VERSION;
    request[1] = slot;
    request[2] = readonly ? 1 : 0;
    request[3] = type;
    put_u16le(request + 4, sector_size_hint);
    put_u16le(request + 6, uri_length);
    memcpy(request + 8, uri, uri_length);
    request_length = (uint16_t)(8 + uri_length);

    result = disk_call(FN_DISK_CMD_MOUNT, request, request_length,
                       reply, sizeof(reply), &reply_length);
    if (result != FN_OK) return result;
    if (reply_length != 12 || reply[0] != FN_DISK_PROTOCOL_VERSION) {
        return FN_ERR_IO;
    }
    if (info) {
        info->flags = reply[1];
...
{
    uint8_t request[2] = { FN_DISK_PROTOCOL_VERSION, slot };
    uint8_t reply[16];
    uint16_t reply_length = 0;
    uint8_t result = disk_call(FN_DISK_CMD_UNMOUNT, request, sizeof(request),
                               reply, sizeof(reply), &reply_length);
    if (result != FN_OK) return result;
    if (reply_length != 5 || reply[0] != FN_DISK_PROTOCOL_VERSION ||
        reply[4] != slot) return FN_ERR_IO;
    return FN_OK;
}
...
uint8_t fn_disk_info(uint8_t slot, fn_disk_info_t *info)
{
    ...
    result = disk_call(FN_DISK_CMD_INFO, request, sizeof(request),
                       reply, sizeof(reply), &reply_length);
    if (result != FN_OK) return result;
    return parse_info(reply, reply_length, info);
}
...
uint8_t fn_disk_clear_changed(uint8_t slot)
{
    ...
    uint8_t reply[16];
    uint16_t reply_length = 0;
    uint8_t result = disk_call(FN_DISK_CMD_CLEAR_CHANGED, request, sizeof(request),
                               reply, sizeof(reply), &reply_length);
    if (result != FN_OK) return result;
    if (reply_length != 5 || reply[0] != FN_DISK_PROTOCOL_VERSION ||
        reply[4] != slot) return FN_ERR_IO;
                            ...
                            uint16_t *data_length)
{
    ...
    request[0] = FN_DISK_PROTOCOL_VERSION;
    request[1] = slot;
    put_u32le(request + 2, lba);
    put_u16le(request + 6, data_capacity);
    result = disk_call(FN_DISK_CMD_READ_SECTOR, request, sizeof(request),
                       reply, sizeof(reply), &reply_length);
    if (result != FN_OK) return result;
    if (reply_length < 11 || reply[0] != FN_DISK_PROTOCOL_VERSION ||
        reply[4] != slot || get_u32le(reply + 5) != lba) return FN_ERR_IO;
    payload_length = get_u16le(reply + 9);
    if ((uint32_t)11 + payload_length != reply_length ||
        payload_length > data_capacity) return FN_ERR_IO;
    memcpy(data, reply + 11, payload_length);
                             ...
                             const uint8_t *data, uint16_t data_length)
{
    ...
    request[1] = slot;
    put_u32le(request + 2, lba);
    put_u16le(request + 6, data_length);
    memcpy(request + 8, data, data_length);
    result = disk_call(FN_DISK_CMD_WRITE_SECTOR, request,
                       (uint16_t)(8 + data_length), reply, sizeof(reply),
                       &reply_length);
    if (result != FN_OK) return result;
    if (reply_length != 11 || reply[0] != FN_DISK_PROTOCOL_VERSION ||
        reply[4] != slot || get_u32le(reply + 5) != lba ||
        get_u16le(reply + 9) != data_length) return FN_ERR_IO;
    return FN_OK;
}
