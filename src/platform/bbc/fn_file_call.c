...
static uint8_t map_file_status(uint8_t status)
{
    switch (status) {
        case 0: return FN_OK;
                         ...
                         uint16_t *response_len)
{
    ...
    block[0] = FN_BBC_REASON_FILE_CALL;
    block[2] = command;
    block[4] = (uint8_t)((unsigned)request & 0xFFu);
    block[5] = (uint8_t)(((unsigned)request >> 8) & 0xFFu);
    block[6] = (uint8_t)(request_len & 0xFFu);
    block[7] = (uint8_t)(request_len >> 8);
    block[8] = (uint8_t)((unsigned)response & 0xFFu);
    block[9] = (uint8_t)(((unsigned)response >> 8) & 0xFFu);
    block[10] = (uint8_t)(response_capacity & 0xFFu);
    block[11] = (uint8_t)(response_capacity >> 8);

    rom_status = fn_bbc_osword78(block);
    *response_len = (uint16_t)block[12] | ((uint16_t)block[13] << 8);
    if (rom_status != FN_BBC_STATUS_OK) {
        return fn_bbc_status_to_result(rom_status);
    }

    return map_file_status(block[3]);
}
